...
static bool update_flg = 0;	//update finished flag
static bool init_flg = 0;	//first update flag

static uint8 pwm_chn_num = 0;
static uint8 pwm_out_io_num[8] = {0};	//each channel gpio number

static bool pwm_stop_flag = 0;       //start/stop frc1 timer

static struct pwm_single_param local_single[8 + 1];	//local_single param, on-changing
static uint8 local_channel = 0;				//local_channel value


static struct pwm_single_param run_pwm_single[2][8 + 1];   //running para,  two sets
static uint8 run_pwm_channel[2];

static uint8 run_pwm_toggle = 0;
static struct pwm_single_param *pwm_single;
static uint8 pwm_channel;

static uint8 pwm_current_channel = 0;	//current pwm channel in pwm_tim1_intr_handler
static uint16 pwm_gpio = 0;		//all pwm gpio bits
